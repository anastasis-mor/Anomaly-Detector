...
const THRESHOLDS = {
  FAILED_LOGIN: 2,           // 10 failed logins in 5 minutes
...
const checkForFailedLoginThreat = async (siteId) => {
  ...
  try {
    ...
    if (count >= THRESHOLDS.FAILED_LOGIN) {
      console.log(`Threshold reached: ${count} failed logins in 5 minutes for site ${siteId}`);
  ...
  const debugLogs = async () => {
    try {
      ...
      if (recentLogs.length > 0) {
        console.log("Sample log fields:", Object.keys(recentLogs[0]._doc));
        console.log("Sample log 1:", JSON.stringify(recentLogs[0], null, 2));
      } else {
        console.log("No logs found");
      }
      
      // Count failed login logs
      const failedLoginCount = await Log.countDocuments({ action: "failed_login" });
      console.log(`Failed login logs: ${failedLoginCount}`);
      
      // Check site field
      const logsBySite = await Log.aggregate([
        { $group: { _id: "$site", count: { $sum: 1 } } }
      ]);
      console.log("Logs by site:", logsBySite);
      
      // Check active sites
      const Site = require('../models/siteModel');
      const activeSites = await Site.find({ status: 'active' });
      console.log(`Active sites: ${activeSites.length}`);
      console.log("Site IDs:", activeSites.map(site => site._id));
